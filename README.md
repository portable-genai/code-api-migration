# Code and API Migration Copilot (H5)

Deterministic repo and API migration analysis with cited, human-reviewed migration plans.
...
code_api_migration analyze legacy-flask-app   # analyse a target repo's migration
```
...
One env var, `CODEMIGRATION_PROFILE`, selects the adapter family:
...
`CODEMIGRATION_ALLOW_INSECURE_DEMO=1` says otherwise. The guard reads the identity
BINDING to decide that, never a service credential: setting
`CODEMIGRATION_S2S_TOKEN` closes the S2S routes and does not open anything else.
See `docs/runbook.md`.
...
| `pii-kit` | the jurisdiction PII pattern pack the migration service redacts with |
...
**Name the profile.** `CODEMIGRATION_PROFILE` has no default. Leaving it unset is
