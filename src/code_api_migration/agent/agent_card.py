...
class AgentCard:
    ...
    provider: str = "code-api-migration"
...
SKILLS: tuple[AgentSkill, ...] = (
    AgentSkill(
        ...
        name="Repo migration analysis",
        description=(
            "Analyse a target repo into a cited migration plan: run the deterministic AST, "
            "dependency and breaking-change engines, record an already-redacted audit event, "
            "and ROUTE the result to human review when it proposes changes (rule R8). Every "
            "finding and step is computed by pure stdlib code, never by a model."
        ),
    ),
    AgentSkill(
...
_DESCRIPTION = " ".join(
    (
        "Code and API Migration Copilot",
        "(H5).",
...
def _resolve_url(settings: Settings) -> str:
    ...
    return f"https://code-api-migration.{settings.region}.internal.example/a2a"
