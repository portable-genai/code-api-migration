...
DEFAULT_ACTOR = "code-api-migration-agent"
...
def analyze_migration(
    ...
    """Analyse a target repo's migration and route it for human review when it proposes changes.

    Resolves the repo through the scanner port, runs the deterministic engines to produce every
    finding, verdict and step, writes an already-redacted audit event, and, when the analysis
    proposes changes (any breaking change), submits the result to the human-review console
    (rule R8). The model narrates nothing here; every number comes from the pure engines.

    Args:
      repo_id: The target repository id the scanner resolves to a checkout.
    ...
    if not isinstance(payload, dict):  # pragma: no cover - dataclasses serialise to objects
        raise TypeError("a migration result must serialise to a JSON object")
...
TOOL_FUNCTIONS = (analyze_migration, verify_audit_trail)
