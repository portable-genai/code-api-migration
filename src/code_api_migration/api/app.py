"""FastAPI application for Code and API Migration Copilot (H5).
...
``CODEMIGRATION_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``CODEMIGRATION_S2S_TOKEN`` is wrong in the most dangerous direction:
...
header and no ``X-Dev-Persona`` then got the seeded persona list and a real migration decision.
...
_TOKEN_ENV = "CODEMIGRATION_S2S_TOKEN"
_INSECURE_DEMO_ENV = "CODEMIGRATION_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "CODEMIGRATION_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "CODEMIGRATION_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "CODEMIGRATION_S2S_AUDIENCE"
_API_HOST_ENV = "CODEMIGRATION_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``CODEMIGRATION_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``CODEMIGRATION_PROFILE`` must never be allowed to
    ...
    ``CODEMIGRATION_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "CODEMIGRATION_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "code-api-migration"
...
_APP_TITLE = "Code and API Migration Copilot"
...
# names is what answered, never what configuration says would. This service binds no model port,
# so its deterministic engine notes nothing and sends neither header; the pill keeps showing the
# configured `generator_model` from `/healthz`.
...
#
# Note what is NOT in this expression: CODEMIGRATION_S2S_TOKEN. A service
...
@app.post("/v1/migrations", response_model=MigrationResponse, tags=["artifacts"])
def analyze_migration(
    request: MigrationRequestModel,
    principal: Annotated[Principal, Depends(get_principal)],
) -> MigrationResponse:
    """Analyse a target repo into a cited migration plan; the actor is the verified principal.
    ...
    # The hand-off never fails an already-computed, already-audited migration; the response
    ...
    return MigrationResponse.from_domain(
        result, plan, review_ref=review_ref, review_routing=routing.outcome.value
    )
