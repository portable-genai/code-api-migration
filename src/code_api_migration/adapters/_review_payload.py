...
from ..domain.models import MigrationResult
...
def _kit_citations(result: MigrationResult) -> tuple[KitCitation, ...]:
    seen: set[str] = set()
...
def result_to_review(result: MigrationResult, *, maker: str, tenant: str = "") -> Review:
    """Build the review a producer submits to human-review-console when a result escalates."""
    return Review(
        action="code_api_migration:migration",
        ...
        sod_group="code_api_migration-maker-checker",
        case_ref=result.subject,
        # Producer-owned, tenant-scoped key so a retried delivery is idempotent at the console.
        source_key=f"code-api-migration:{result.subject}:{result.severity.value}",
