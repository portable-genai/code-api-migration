...
(``CODEMIGRATION_REVIEW_ROUTING=off``), the container binds :class:`DisabledReviewRouter` instead
...
rather than failing an already-computed, already-audited migration, but it is never invisible: the
...
class DisabledReviewRouter:
    ...
    def route(self, result: MigrationResult, *, maker: str, tenant: str = "") -> str:
        return ""
