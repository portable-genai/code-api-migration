...
One env var (``CODEMIGRATION_PROFILE``) selects the adapter family for every
...
from .ports.ci_status import CiStatusPort
...
from .ports.repo_access import RepoAccessPort
from .ports.repo_scanner import RepoScannerPort
...
class ProfileChoice:
    ...
    The variable is ``CODEMIGRATION_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("CODEMIGRATION_PROFILE", "local")``: that fallback reads an
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "repo_scanner": {
        "local": f"{_PKG}.adapters.local.repo_scanner:LocalRepoScanner",
        "gcp": f"{_PKG}.adapters.gcp.repo_scanner:CloudRepoScanner",
        "onprem": f"{_PKG}.adapters.onprem.repo_scanner:OnPremRepoScanner",
    },
    "repo_access": {
        "local": f"{_PKG}.adapters.local.repo_access:LocalRepoAccess",
        "gcp": f"{_PKG}.adapters.gcp.repo_access:CloudRepoAccess",
        "onprem": f"{_PKG}.adapters.onprem.repo_access:OnPremRepoAccess",
    },
    "ci_status": {
        "local": f"{_PKG}.adapters.local.ci_status:LocalCiStatus",
        "gcp": f"{_PKG}.adapters.gcp.ci_status:CloudCiStatus",
        "onprem": f"{_PKG}.adapters.onprem.ci_status:OnPremCiStatus",
    },
}
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``CODEMIGRATION_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "CODEMIGRATION_REVIEW_ROUTING"
...
class Container:
    ...
    @cached_property
    def repo_scanner(self) -> RepoScannerPort:
        adapter = self._bind("repo_scanner")
        assert isinstance(adapter, RepoScannerPort)
        return adapter

    @cached_property
    def repo_access(self) -> RepoAccessPort:
        adapter = self._bind("repo_access")
        assert isinstance(adapter, RepoAccessPort)
        return adapter

    @cached_property
    def ci_status(self) -> CiStatusPort:
        adapter = self._bind("ci_status")
        assert isinstance(adapter, CiStatusPort)
        return adapter
