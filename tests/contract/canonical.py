...
CANONICAL_EVENT = AuditEvent(
    ...
    redacted_summary="legacy-flask-app: flask migration plan with 2 step(s)",
    citations=(Citation(source_id="flask-2x", title="Flask 2.x notes", snippet="run() keywords"),),
)

#: The escalated result every review-router implementation is handed (rule R8's payload).
CANONICAL_RESULT = MigrationResult(
    subject=sample_cases.ESCALATING_REPO_ID,
    severity=Severity.HIGH,
    decision=Decision.ESCALATED,
    summary=f"{sample_cases.ESCALATING_REPO_ID}: flask migration plan with 2 step(s)",
...
#: A consequential write intent every repo-access implementation is handed.
CANONICAL_INTENT = PullRequestIntent(
    repo_id=sample_cases.ESCALATING_REPO_ID,
...
def _scanner_invoke(adapter: Any) -> Any:
    return adapter.scan(sample_cases.ESCALATING_REPO_ID)


def _scanner_answered(_adapter: Any, result: Any) -> bool:
    return isinstance(result, RepoCheckout) and bool(result.files) and bool(result.framework)


def _access_invoke(adapter: Any) -> Any:
    return adapter.open_pull_request(CANONICAL_INTENT)


def _access_answered(adapter: Any, result: Any) -> bool:
    return bool(result) and len(adapter.opened) == 1


def _ci_invoke(adapter: Any) -> Any:
    return adapter.latest_status(sample_cases.ESCALATING_REPO_ID, "refs/heads/main")


def _ci_answered(_adapter: Any, result: Any) -> bool:
    return isinstance(result, CiStatus) and bool(result.state)
...
CANONICAL_CALLS: dict[str, PortCase] = {
    ...
    "repo_scanner": PortCase(
        invoke=_scanner_invoke,
        answered=_scanner_answered,
        # The managed scanner's first act is the lazy `google.cloud` import.
        managed_refusal=(ImportError,),
        detail="assemble one target repository checkout",
    ),
    "repo_access": PortCase(
        invoke=_access_invoke,
        answered=_access_answered,
        # The managed writer's first act is the lazy `google.cloud` import.
        managed_refusal=(ImportError,),
        detail="record one proposed pull request",
    ),
    "ci_status": PortCase(
        invoke=_ci_invoke,
        answered=_ci_answered,
        # The managed CI reader's first act is the lazy `google.cloud` import.
        managed_refusal=(ImportError,),
        detail="read one CI status",
    ),
    "tracer": PortCase(
