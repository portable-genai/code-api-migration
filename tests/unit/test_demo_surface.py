...
def test_the_demo_data_is_obviously_fictional() -> None:
    checkouts = (demo.ROUTINE_CHECKOUT, demo.ESCALATING_CHECKOUT, demo.PII_CHECKOUT)
    for checkout in checkouts:
        assert "FICTIONAL" in checkout.repo_id, checkout.repo_id + " must announce itself as fake"
    blob = " ".join(checkout.changelog for checkout in checkouts) + demo.ACTOR
    ...
    # RFC 3849 documentation literal, never a routable address.
    assert "2001:db8:" in blob
