...
from code_api_migration.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from code_api_migration.config import Settings, _read_settings_file
from code_api_migration.ports.identity import (
    VERIFIED,
...
_PROFILE_ENV = "CODEMIGRATION_PROFILE"
_SETTINGS_ENV = "CODEMIGRATION_SETTINGS"

#: The API module, named once so no line's length depends on the rendered package name.
_API_MODULE = "code_api_migration.api.app"
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        *[
        ...
        # The non-identity ports are rebound to their LOCAL adapters under gcp too, so the served
        # request exercises the REAL IAP identity verifier while the scanner, writer and CI reader
        # answer offline (the documented rebinding path, not a test-only hook).
        "  repo_scanner:",
        *[
            f"    {p}: {_PKG}.adapters.local.repo_scanner:LocalRepoScanner"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.repo_scanner:OnPremRepoScanner",
        "  repo_access:",
        *[f"    {p}: {_PKG}.adapters.local.repo_access:LocalRepoAccess" for p in ("local", "gcp")],
        f"    onprem: {_PKG}.adapters.onprem.repo_access:OnPremRepoAccess",
        "  ci_status:",
        *[f"    {p}: {_PKG}.adapters.local.ci_status:LocalCiStatus" for p in ("local", "gcp")],
        f"    onprem: {_PKG}.adapters.onprem.ci_status:OnPremCiStatus",
    ]
)
...
def _managed_app(monkeypatch: pytest.MonkeyPatch, tmp_path: Any, audience: str | None) -> Any:
    ...
    supplies it (three-state: unset takes the written default, an EMPTIED variable is refused by
    the loader and never resolves to empty). ``None`` therefore means UNSET, which is how a
    deployment says "no audience configured"; blanking the variable is a different state with a
    different, louder answer, pinned by
    ``test_an_emptied_audience_is_refused_by_the_settings_loader``.
    """
...
def test_an_emptied_audience_is_refused_by_the_settings_loader(
    ...
    """UNSET and EMPTIED are different states, and the loader is where the difference is kept.

    `iap_audience: ${CODEMIGRATION_IAP_AUDIENCE:-}` is a documented default with a variable
    allowed to override it, so it obeys the same rule as `setting_or_default`: an operator who
    emptied the variable expressed an intent that names nothing, and it must not silently become
    "no audience configured". The refusal happens at load, naming the variable.
    """
    path = tmp_path / "settings.yaml"
    path.write_text(_REBOUND_SETTINGS + "\n", encoding="utf-8")
