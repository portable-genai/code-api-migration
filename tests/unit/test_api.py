...
def test_migration_uses_the_verified_principal_as_actor(api_client: TestClient) -> None:
    ...
    assert body["steps"], "a proposed migration must carry ordered steps"
    assert body["findings"], "the audit view needs the full finding set"
    # Rule R8: the escalation was routed, not merely flagged (see test_review_routing.py).
    assert body["review_ref"]


def test_a_clean_repo_does_not_escalate(api_client: TestClient) -> None:
    resp = api_client.post(
        "/v1/migrations",
