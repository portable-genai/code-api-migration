...
the three-state expansion rule, so a value an operator deliberately emptied is refused rather
than inheriting the default written in the file.
"""
...
def test_an_env_var_set_to_empty_does_not_inherit_the_files_default(
    ...
    # Resolving to empty would make `${VAR:-fallback-region}` with VAR="" indistinguishable
    # from `${VAR:-}`, so the operator who emptied it gets a value nobody reviewed. The loader
    # is the last place that still knows a default was written, so it refuses here.
...
def test_a_bare_env_ref_set_to_empty_is_refused(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """`${VAR}` with no `:-` is `setting_or_default(VAR, "")`: unset is empty, emptied refuses."""
    path = tmp_path / "settings.yaml"
