...
from code_api_migration.agent import (
    SKILLS,
    TOOL_FUNCTIONS,
    agent_card_document,
    analyze_migration,
...
def test_the_card_document_is_json_safe_and_names_the_region() -> None:
    ...
    assert document["name"] == "code-api-migration"
...
def test_the_agent_path_routes_an_escalation_rather_than_only_flagging_it() -> None:
    result = analyze_migration(
        sample_cases.ESCALATING_REPO_ID,
...
def test_the_agent_path_does_not_manufacture_a_review_for_a_routine_case() -> None:
    result = analyze_migration("tidy-app", settings=local_settings())
...
def test_the_tool_output_is_masked_before_it_can_reach_a_model() -> None:
    ...
    caller the changelog metadata it is authorised to see. The difference is the destination,
    not the data: the escalating fixture's changelog carries a synthetic email and NRIC, and the
    tool masks both on the way to the model.
    """
    result = analyze_migration(sample_cases.ESCALATING_REPO_ID, settings=local_settings())
