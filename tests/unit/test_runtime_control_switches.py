...
control this service has: ``CODEMIGRATION_REVIEW_ROUTING`` is read in three states; off binds a
...
an already-computed migration when the console is unreachable.
"""
...
from code_api_migration.adapters.controls import (
    DisabledReviewRouter,
    RecordingReviewRouter,
    ReviewRouting,
)
from code_api_migration.agent import tools
from code_api_migration.api import app as api_module
from code_api_migration.api.app import app
from code_api_migration.cli import main as cli
...
from code_api_migration.envread import ConfiguredEmptyError

_LOOPBACK = ("127.0.0.1", 50000)
_PATH = "/v1/migrations"
_ESCALATING: dict[str, Any] = {"repo_id": "legacy-flask-app"}
_ROUTINE: dict[str, Any] = {"repo_id": "tidy-app"}
_HEADERS = {"X-Dev-Persona": "auditor"}
_LOCAL_ROUTER = "code_api_migration.adapters.local.review_router.LocalReviewRouter.route"
...
def _managed(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "code_api_migration.config.resolve_profile",
...
def test_the_off_posture_is_logged_once_however_many_containers(
    ...
    settings = Settings(profile="local", controls=ControlSwitches(review_routing=False))
    with caplog.at_level(logging.WARNING, logger="code_api_migration.config"):
        for _ in range(3):
...
def test_a_failed_hand_off_is_reported_and_logged_never_raised(
    ...
    failed = RecordingReviewRouter(_Refusing())
    with caplog.at_level(logging.WARNING, logger="code_api_migration.adapters.controls"):
...
def test_the_agent_tool_reports_the_hand_off() -> None:
    payload = tools.analyze_migration("legacy-flask-app", tenant="demo-bank")
...
def test_the_cli_reports_the_hand_off(
    ...
    monkeypatch.setenv(REVIEW_ROUTING_ENV, "off")
    assert cli.main(["analyze", "legacy-flask-app"]) == 0
