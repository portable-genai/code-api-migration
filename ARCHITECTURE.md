# ARCHITECTURE: Code and API Migration Copilot (H5)

Hexagonal ports-and-adapters. A pure-stdlib domain core speaks only to ports (`typing.Protocol`s);
adapter families implement them; one env var (`CODEMIGRATION_PROFILE`) swaps the
...
## Layout (`src/code_api_migration/`)
- `domain/` : pure stdlib, no cloud/framework imports. `kernel.py` (vertical-neutral types,
  `StrEnum` taxonomies from the commons), `models.py` (the migration artifacts: findings, plans,
  results), `pii.py` (the jurisdiction pattern selection + order), and the pure engines
  `ast_engine.py`, `dependency_graph.py`, `breaking_change_engine.py`, `plan_engine.py`,
  `patch_engine.py` composed by `migration_service.py` (packs loaded via `pack_loader.py`).
- `ports/` : `@runtime_checkable` Protocols (`AuditSinkPort`, `ReviewRouterPort`, `RepoScannerPort`,
  `RepoAccessPort`, `CiStatusPort`; identity uses
...
## Request pipeline (`MigrationService.run`, then the caller)
