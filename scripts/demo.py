...
below drives the actual migration service, the actual hash-chained audit store and the actual
...
from code_api_migration.domain import (
    kernel,
    models,
)
...
SERVICE_NAME = "Code and API Migration Copilot"
CATALOG_ID = "H5"
REPOSITORY = "code-api-migration"
...
#: A clean checkout: already on the target framework, so no breaking change and no escalation.
ROUTINE_CHECKOUT = models.RepoCheckout(
    repo_id="beta-tidy (FICTIONAL)",
    framework="flask",
    files=(models.SourceFile(path="calc.py", content="def add(a, b):\n    return a + b\n"),),
)

#: A checkout with a real Flask 1.x breaking change: app.run keeps the removed positional
#: signature, so the engine raises a FAIL finding and a migration is proposed.
ESCALATING_CHECKOUT = models.RepoCheckout(
...
#: The same shape, but its changelog carries commit metadata: an author email and a synthetic
#: national id that must be masked before the audit write and before any model hand-off.
PII_CHECKOUT = models.RepoCheckout(
    repo_id="gamma-legacy (FICTIONAL)",
    framework="flask",
    files=ESCALATING_CHECKOUT.files,
    changelog=(
        "release note authored by ops@gamma.example, NRIC " + PLANTED_NRIC + " on host 2001:db8::7"
    ),
)
...
class DemoRun:
    ...
    def _step_routine(self) -> Produced:
        return self._migration_panels(ROUTINE_CHECKOUT, expect_routing=False)

    def _step_escalation(self) -> Produced:
        return self._migration_panels(ESCALATING_CHECKOUT, expect_routing=True)

    def _step_redaction(self) -> Produced:
        panels, facts = self._migration_panels(PII_CHECKOUT, expect_routing=True)
    ...
    def _migration_panels(self, checkout: models.RepoCheckout, *, expect_routing: bool) -> Produced:
        result, plan = self.service.run(checkout, actor=ACTOR)
        self.cases += 1
        review_ref = ""
        ...
        decision = Panel(
            ...
            rows=(
                ...
                Row("Breaking changes (FAIL)", str(result.fail_count)),
                Row("Needs info", str(result.needs_info_count)),
                Row("Plan steps", str(result.step_count)),
                Row("Requires human review", str(result.requires_human_review)),
                Row(
                    "Routed to review",
                    review_ref or "not routed (clean)",
                    "ok" if consistent else "bad",
                ),
                Row("Attributed to", ACTOR),
            ),
            note=(
                "Every finding, verdict and step is computed by pure stdlib engines and is "
                "replayable. A model would narrate this plan; it never produces it."
            ),
            tone="ok" if consistent else "bad",
        )
        steps = Panel(
            title="Ordered migration plan",
            rows=tuple(
                Row("Step " + str(step.order) + ": " + step.module, step.rationale)
                for step in plan.steps
            )
            or (Row("steps", "none (already migrated)", "ok"),),
            note=(
                "Steps run dependency-first (a module is migrated only after everything it "
                "imports), and every FAIL finding lands in exactly one step."
            ),
        )
        evidence = Panel(
            title="Evidence",
            ...
            or (Row("citations", "none (clean repo)", "ok"),),
            note="Every finding carries its source. An uncited claim is a hallucination risk.",
        )
        facts = {
        ...
        return [decision, steps, evidence], facts
...
def _exit_audit(container: Any) -> Any:
    return container.audit.record(
        kernel.AuditEvent(
            ...
            redacted_summary="acme-legacy (FICTIONAL): flask migration plan",
        )
    )
...
def _exit_review(container: Any) -> Any:
    citation = kernel.Citation(
        ...
        snippet="app.run positional signature removed",
    )
    return container.review_router.route(
        models.MigrationResult(
            subject="acme-legacy (FICTIONAL)",
            severity=kernel.Severity.HIGH,
            decision=kernel.Decision.ESCALATED,
            summary="acme-legacy (FICTIONAL): flask migration plan",
...
def _exit_repo_scanner(container: Any) -> Any:
    return container.repo_scanner.scan("acme-legacy")


def _exit_repo_access(container: Any) -> Any:
    return container.repo_access.open_pull_request(
        models.PullRequestIntent(
            repo_id="acme-legacy",
            branch="migrate/flask-2x",
            title="Migrate to Flask 2.x",
            body="Applies the reviewed migration plan.",
        )
    )


def _exit_ci_status(container: Any) -> Any:
    return container.ci_status.latest_status("acme-legacy", "refs/heads/main")
...
#: portability tour that toured every seam except that one was skipping the seam whose exit
#: behaviour matters most. REPO_ACCESS is the consequential write, and it must refuse loudly too.
EXIT_CALLS: dict[str, Callable[[Any], Any]] = {
    ...
    "repo_scanner": _exit_repo_scanner,
    "repo_access": _exit_repo_access,
    "ci_status": _exit_ci_status,
}
