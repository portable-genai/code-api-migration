...
Rewrites the python package name, the console-script name, the ``CODEMIGRATION`` env-var prefix,
...
Markdown prose alone (pass ``--include-docs`` to sweep it too).
    ...
    # Apply it:
    python scripts/rename_fork.py --package acme_migration --cli acme-migrate \
        --env-prefix ACME --resource acme-migrate --yes
...
# --------------------------------------------------------------------------------------- #
_OLD_PACKAGE = "code_api_migration"
_OLD_CLI = "code_api_migration"
_OLD_ENV_STEM = "CODEMIGRATION"
_OLD_RESOURCE = "h5-svc"
_OLD_DIST = "code-api-migration"
...
def _rules(args: argparse.Namespace) -> list[tuple[str, str, str]]:
    ...
    return [
        ...
        # The env-var prefix. Matched WITHOUT the trailing underscore so both CODEMIGRATION_PROFILE
...
def main() -> int:
    ...
    parser.add_argument("--cli", required=True, help="new console-script name, e.g. acme-migrate")
    parser.add_argument("--env-prefix", required=True, help="new env-var prefix stem, e.g. ACME")
    parser.add_argument(
        "--resource", required=True, help="new Terraform name_prefix stem, e.g. acme-migrate"
    )
    parser.add_argument(
