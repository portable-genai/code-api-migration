...
A Next.js App Router console for Code and API Migration Copilot (H5).
...
| `app/page.tsx` | The console itself. It posts the chosen repository id to `POST /v1/migrations` (`{ repo_id }`, the whole request model) and offers the local profile's fictional fixture ids. It reads the service's agent card for its own title, so no product name is hardcoded here. |
