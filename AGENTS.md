...
Code and API Migration Copilot (H5): Deterministic repo and API migration analysis with cited, human-reviewed migration plans.
Rendered from `hex-service-template`, so it starts at reference parity rather than converging
toward it. Package `code_api_migration`, environment prefix
`CODEMIGRATION`, region `asia-southeast1`.
...
- `src/code_api_migration/domain/` is PURE stdlib. `kernel.py` holds the vertical-neutral
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `CODEMIGRATION_S2S_TOKEN` may never enter that decision: it authenticates a
  ...
  persona and a real migration decision. `tests/unit/test_end_user_auth_posture.py` walks the
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `CODEMIGRATION_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `CODEMIGRATION_PROFILE` (mentioning it in a refusal message is wanted, reading
