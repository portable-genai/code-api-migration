# SPEC: Code and API Migration Copilot (H5)
...
- **Redaction before audit**: the migration service redacts PII (via `pii-kit`) before writing any
  audit record. No raw identifier reaches the WORM store.
- **Determinism**: the overall severity band, every breaking-change finding and every plan step
  are pure stdlib and replayable; an LLM may narrate but never produces a finding or a step. The
  engine chain is `ast_engine` (stdlib `ast` facts) -> `dependency_graph` (topological order,
  refusal on cycles) -> `breaking_change_engine` (per-framework YAML packs of falsifiable machine
  rules returning PASS / FAIL / NEEDS_INFO / NOT_APPLICABLE) -> `plan_engine` (every FAIL finding
  in exactly one step, dependency-first) -> `patch_engine` (a drafted diff is applied in memory
  and re-analysed; it is demoted to draft unless it applies, clears its finding and still parses).
- **Packs are configuration**: breaking-change rules live in versioned YAML under `config/packs/`,
  loaded and strictly validated by `pack_loader.py` (unknown fields, kinds or frameworks refuse at
  load). Adding a migration is a pack, not a code edit.
- **Repo ports**: the target checkout arrives through `RepoScannerPort` (local fixture repos under
  `fixtures/repos/`), the consequential branch/PR write through `RepoAccessPort`, and CI state
  through `CiStatusPort`. Opening a PR is consequential and requires an approved review first (R8);
