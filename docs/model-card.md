...
Hrz4 promotion gate records a verdict AGAINST, not a model this service calls.
...
- **Escalation is unchanged.** A consequential result still sets `requires_human_review` and is
  still ROUTED to Hrz7 through `ReviewRouterPort` in the same call (rule R8).
...
2. The exact model id and version pinned in this card, in the same commit that binds it, and kept
   in step with `_GATED_MODEL` so an Hrz4 verdict cannot be inherited across a model swap.
...
4. An evaluation that scores the LIVE model, not only the stub: the offline gate measures the
   deterministic engines, so a managed-profile run through the Hrz4 promotion gate has to score
   the narration itself.
5. Prompt-injection screening on the input, through the **Hrz1** guardrail gateway (COMPLIANCE row
